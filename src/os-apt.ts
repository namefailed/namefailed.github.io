...
function installFinale(pkgDisplay: string, outcome: AptInstallOutcome): string[] {
...
export function runApt(args: string[]): string[] {
  ...
  if (sub === 'list' || sub === 'ls') {
    ...
    if (pkgs.length === 0)
      ...
      return [`  ${c.dim}(empty — try ${c.blue}apt install cowsay${c.reset}${c.dim})${c.reset}`]
  ...
  if (sub === 'install') {
    ...
    if (!raw) return [`  ${c.red}E:${c.reset} ${c.dim}need a package name.${c.reset}`]



    const display = raw

    return [...installPageantry(display), ...installFinale(display, attemptAptInstall(raw))]
  ...
  if (sub === 'remove' || sub === 'purge') {
    ...
    if (!pkg) return [`  ${c.red}E:${c.reset} ${c.dim}need a package name.${c.reset}`]
  ...
  if (sub === 'search' || sub === 'find') {
    ...
    const shelf: readonly Row[] = [
      ...
      {
        id: 'docker-for-tabs',
        blurb: 'hypothetical — each browser tab already is a container spiritually',
      },
      {
        id: 'systemd-simp',
    ...
    const formatBlurb = (r: Row): string => {
      ...
      if (r.id === 'neofetch-hallucination') {
        ...
        return `${c.dim}blocked by ergonomics — try ${c.blue}whoami -a${c.reset}${c.dim} instead.${c.reset}`
    ...
    if (!hits.length)
      ...
      return [`  ${c.dim}no phantom packages match ${c.blue}${needle || '∅'}${c.reset}`, '']

    const lines: string[] = [
      '',
      `  ${c.dim}browsing repo.mrgrey.dev/imaginary …${c.reset}`,
      '',
      ...hits.map(r => `  ${c.green}${r.id.padEnd(22)}${c.reset}  ${formatBlurb(r)}`),
      '',
  ...
  return [`  ${c.red}E:${c.reset} Unknown — try ${c.blue}apt${c.reset}${c.dim} alone.${c.reset}`]
