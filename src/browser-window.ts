...
export class BrowserWindow {
  ...
  constructor(opts: BrowserWindowOptions) {
    ...
    const mkBtn = (label: string, title: string, fn: () => void): HTMLButtonElement => {
      ...
      b.type = 'button'
      b.className = 'browser-btn os-toolbar-btn'
      b.textContent = label
      b.title = title
    ...
    const btnBack = mkBtn('←', 'Back', () => this.tryHistory(-1))
    const btnFwd = mkBtn('→', 'Forward', () => this.tryHistory(1))
    ...
    const btnGo = mkBtn('Go', 'Go', () => this.goFromInput())
    ...
    for (const bm of BROWSER_BOOKMARKS) {
      ...
      bk.type = 'button'
      bk.className = 'browser-bookmark os-toolbar-btn'
  ...
  private openIframeTip(): void {
    ...
    dialog.innerHTML = `
      ...
      <p class="browser-iframe-tip-body browser-iframe-tip-body--muted">
        Use <strong>↗</strong> in the toolbar to open the URL in a normal tab. Same-origin pages (this site) work in-frame.
      </p>
